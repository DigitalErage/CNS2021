...
parser.add_argument('--d_r', metavar='%', type=float, default=0.99,
                    help='detection rate for state_manupulation_attack')

args = parser.parse_args()

evals = np.load('evals/revals_%.2f,%.1f,%d.npy'%(args.ap,args.x,args.l))
...
def train(steps, device):
    ...
    for i in range(steps):
        ...
        max_reward_list.append(total_reward)
...
def multi_train(train_times, process_num, gpu_num):#for best performance, please set train_times % 6 == 0
    ...
    with open("results/dqn/dqn_results_%.2f,%.1f,%d,%d,%.2f,%.2f.txt"%(args.ap,args.x,args.l,args.b,args.p_s,args.d_r), "wb") as fp:   #fc, sgc
